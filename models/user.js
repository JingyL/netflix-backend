  ...
  static async get(username) {
    ...
    const userMovieList = await db.query(
      `SELECT m.movie_id, m.movie_name
       FROM movielists AS m
       WHERE m.username = $1`, [username]);

user.movielist = userMovieList.rows.map(m => [m.movie_id, m.movie_name]);
...
//  will add one more here, allow users to add movie to list
...
static async addToList(username, movie_name, movie_id) {
  ...
  await db.query(
        `INSERT INTO movielists (username, movie_name, movie_id)
         VALUES ($1, $2, $3)`,
